...
function callUserActivityLogs(callFor,roleModuleId){
	...
	if(callFor=='activity-logs'){
		...
		callForDashboardData('formIdIfAny','remarks-history-content?sessionUserId='+USER_ID+'&userId='+userId+'&moduleId='+roleModuleId+'&tt=theme1','userActivityContent');
	}else if(callFor=='login-history'){
		$('#activityLogTab').addClass('inactive-tab');
		$('#activityHistoryTab').addClass('active');
		callForDashboardData('formIdIfAny','attendance-content?userId='+userId+'&type=modal&moduleId='+roleModuleId+'&tt=theme1', 'userActivityContent');
	}
}
