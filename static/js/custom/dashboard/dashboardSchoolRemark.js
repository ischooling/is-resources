...
function redirectToBankDetailsOnProfileRequest(){
	if($('#profileApprovalId #remarksStatus').val()=="3"){
		...
		var userId = $('#profileApprovalModal #userId').val();
		var agreementContent = $(".agreementContent-"+userId).html();
		if(agreementContent!=undefined && agreementContent!=''){
			// initEditor(1, 'techerAgreementTinymce',agreementContent, true);
			initEditor(1, 'techerAgreementTinymce',' ', true);
			setTimeout(function(){
				editor1.setData(agreementContent);
			},200)
		}else{
