  ...
  $("#" + elementId).DataTable({
    ...
    ajax: {
      url:
        CONTEXT_PATH +
        SCHOOL_UUID +
        "/dashboard/manage-profile-content-1/" +
        UNIQUEUUID +
        "" +
        argument,
      data: function (data) {
...
function showManageProfileStudentContentListingWithQueries(
  elementId,
  argument,
  userRole
) {
  ...
  var columns = [];
  columns.push({ data: "sno", name: "sno", title: "S.No" });
  if (SCHOOL_ID == 5) {
    columns.push({ data: "groupName", name: "groupName", title: "Group Name" });
  }
  columns.push(
    ...
    //	        { "data": "standard", "name" : "standard" , "title" : "Grade"},
    {
      data: "profileStatus",
      name: "profileStatus",
      title: "SMS/LMS Profile Status",
    },
    {
      data: "createDate",
      name: "createDate",
      title: "Last Profile Updated Date",
    },
    {
    ...
    columns.push(
      {
        data: "referralCode",
        name: "referralCode",
        title: "Counselor Name/Referral Code",
      },
      { data: "action", name: "action", title: "Action" }
    );
  } else {
  ...
  $("#" + elementId).DataTable({
    ...
    fnRowCallback: function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
      if (
        aData["paymentTypeApplicationFee"] ||
        aData["parentProfile"] ||
        aData["viewProfile"]
      ) {
        $("td", nRow).css("text-align", "left");
  ...
  if (userRole == "SCHOOL") {
    $("#" + elementId)
      .dataTable()
      .fnSetColumnVis(6, false);
    $("#" + elementId)
      .dataTable()
      .fnSetColumnVis(7, false);
  }
  $("#" + elementId)
    .dataTable()
    .fnSetFilteringEnterPress();
...
// function showManageProfileTeacherContentListingWithQueries(
//   elementId,
//   argument
// ) {
//   console.log("DEFAULT_SEARCH_STATE: " + DEFAULT_SEARCH_STATE);
...
//       { data: "employeeType", name: "employeeType", title: "Employee Type" },
//       {
//         data: "assignCourse",
...
function showManageProfileCommonContentListingWithQueriesWithoutGoto(
  elementId,
  argument
) {
  console.log("DEFAULT_SEARCH_STATE: " + DEFAULT_SEARCH_STATE);
  ...
  $("#" + elementId).DataTable({
    ...
    columns: [
      ...
      { data: "referralCode", name: "referralCode", title: "Referral Code" },
      { data: "profileStatus", name: "profileStatus", title: "Profile Status" },
...
function showManageProfileCommonContentListingWithQueries(elementId, argument) {
  ...
  $("#" + elementId).DataTable({
    ...
    columns: [
      ...
      { data: "roleName", name: "roleName", title: "Role" },
      {
        data: "gotoMeetingStatus",
        name: "gotoMeetingStatus",
        title: "Goto Meeting User",
      },
      { data: "referralCode", name: "referralCode", title: "Referral Code" },
      { data: "profileStatus", name: "profileStatus", title: "Profile Status" },
      { data: "addedDate", name: "addedDate", title: "Added Date" },
...
function getCallRequestForadvanceTeacherSearch(formId, moduleId) {
  ...
  //   teacherDetailsDTO["profileFor"] = "teacher";
  // }
  teacherDetailsDTO["timZoneFrom"] = $(
    "#" + formId + " #teacherCountryTimezoneFromId option:selected"
  ).attr("data-timezone");
  teacherDetailsDTO["timZoneTo"] = $(
    "#" + formId + " #teacherCountryTimezoneToId option:selected"
  ).attr("data-timezone");
  ...
  teacherDetailsDTO["enrollStatus"] = $(
    "#" + formId + " #filterEnrollStatus"
  ).val();
  teacherDetailsDTO["employeeType"] = $(
    "#" + formId + " #filterEmployeeType"
  ).val();
  teacherDetailsDTO["userClickFrom"] = $(
    "#" + formId + " #userClickFrom"
  ).val();
