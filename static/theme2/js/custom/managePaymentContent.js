	...
	$.each(data.advancePaymentSearchResponseDTO, function (k, apsrSingle) {
		...
		if (roleAndModule.added == 'Y' || roleAndModule.updated == 'Y') {
			...
			if (apsrSingle.isPaymentFailed != null && apsrSingle.paymentStatus != "SUCCESS") {
				html += '<span class="btn btn-sm btn-primary  mr-1" onclick="return showWarningMessageShow(\'Are you sure you want to approve payment and send mails?\',\'callStudentRedirectToDashboard(' + apsrSingle.userId + ',' + apsrSingle.userPaymentDetailsId + ',\\\'Y\\\', function(){ var table = $(\\\'#advPaymentSearch\\\').DataTable(); table.row($(\\\'#row_' + k + '\\\')).remove().draw(false); showMessageTheme2(1, \\\'Record is successfully removed\\\', \\\'\\\', true); })\',false);">Approve Payment With Mail</span>';
				html += '<span class="btn btn-sm btn-primary  mr-1" onclick="return showWarningMessageShow(\'Are you sure you want to approve payment witout sending mails?\',\'callStudentRedirectToDashboard(' + apsrSingle.userId + ',' + apsrSingle.userPaymentDetailsId + ',\\\'N\\\', function(){ var table = $(\\\'#advPaymentSearch\\\').DataTable(); table.row($(\\\'#row_' + k + '\\\')).remove().draw(false); showMessageTheme2(1, \\\'Record is successfully removed\\\', \\\'\\\', true); })\',false);">Approve Payment Without Mail</span>';
			}
		}
		html += '</td></tr>';
	});
