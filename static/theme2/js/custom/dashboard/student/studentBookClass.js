...
function getDetailsForStudentBookaClass(studentStandardId){
    return new Promise(function(resolve, reject){
            ...
            $.ajax({
                ...
                url: getURLForHTML('dashboard', 'student-book-a-class-details?payload='+encode(JSON.stringify(data))),
                dataType : 'json',
                cache : false,
                timeout : 600000,
...
function changeElementrySubject(src, roleModuleId, studentStandardId, teacherAssignFlag, weekLeftClassCount){
	var subjectId = $(src).val();
	if(subjectId != ""){
		$("#viewElementrySlot").removeClass("disabled");
		$("#viewElementrySlot").attr("onclick","bookingSlotModalNew("+`${subjectId}`+","+`${roleModuleId}`+","+`${studentStandardId}`+","+teacherAssignFlag+","+weekLeftClassCount+")");
	}else{
...
function bookingSlotModalForElementry(subjectId, roleModuleId, studentStandardId, teacherAssignFlag,weekLeftClassCount, assignedTeacherCount) {
	if(!teacherAssignFlag){
		showMessageTheme2(2, 'You do not have a teacher assigned yet', '', true);
		return false;
	}
	if(weekLeftClassCount<1){
		showMessageTheme2(2, 'You have booked all your classes for this week', '', true);
		return false;
	}
	var data = {};
	data['userId'] = USER_ID;
	...
	$.ajax({
		type: "GET",
		contentType: "application/json",
		url: getURLForHTML('dashboard', 'student-subject-session-booking-elementry?payload='+encode(JSON.stringify(data))),
		dataType: 'json',
		cache: false,
		timeout: 600000,
			...
			if(data['status'] == '0' || data['status'] == '2' || data['status'] == '3') {
				...
				if($("#weeklyBookClassModal").length > 0){
					$("#weeklyBookClassModal").remove();
					 $("body").append(weeklyBookClassModal(data, subjectId, studentStandardId, roleModuleId, "modal-md",weekLeftClassCount, assignedTeacherCount));
				}else{
					$("body").append(weeklyBookClassModal(data, subjectId, studentStandardId, roleModuleId, "modal-md",weekLeftClassCount, assignedTeacherCount));
				}
				...
				$("#weeklyClassSlotTable .custom-radio-label-tag .custom-control-input").on("change", function(){
					if($(this).is(":checked")){
...
async function bookingSlotModalNew(subjectId, roleModuleId, studentStandardId, teacherAssignFlag, weekLeftClassCount) {
	if(!teacherAssignFlag){
		showMessageTheme2(2, 'You do not have a teacher assigned yet', '', true);
		return false;
	}
    ...
    try {
        const response = await $.ajax({
            type: "GET",
            contentType: "application/json",
            url: getURLForHTML('dashboard', 'student-subject-session-booking-new?payload=' + encode(JSON.stringify(data))),
            dataType: 'json',
            cache: false,
            timeout: 600000
        });
        ...
        if (response['status'] == '0' || response['status'] == '2' || response['status'] == '3') {
            ...
            if (response['teacherAssigned'] === 'YES') {
                ...
                var availabilityData = await getTeacherTimeToShowWeeklyCalendar(subjectId, response['assignedTeacherUserId'], 'STUDENT', 'WEEK', response['studentStandardId'], currentDate);

                if ($("#weeklyBookClassModal").length > 0) {
                    $("#weeklyBookClassModal").remove();
                }

                $("body").append(weeklyBookClassModal(availabilityData, subjectId, studentStandardId, roleModuleId, "modal-xl"));
                $('#weeklyBookClassModal').modal('show');
...
function bookedCalssCotentFun(subjectId, moduleId, studentStandardId, flag, teacherAssignFlag, weekLeftClassCount,assignedTeacherCount) {
		// if(!teacherAssignFlag){
		// 	showMessageTheme2(2, 'You do not have a teacher assigned yet', '', true);
		// 	return false;
		// }
		...
		data['startDate'] = $("#bookClassOneToOne #startDate").val()!=undefined?$("#bookClassOneToOne #startDate").val():'';
		data['endDate'] = $("#bookClassOneToOne #endDate").val()!=undefined?$("#bookClassOneToOne #endDate").val():'';
			...
			success: function (data) {
				...
				if (data['status'] == '0' || data['status'] == '2' || data['status'] == '3') {
					...
					if(flag == "filter" && flag != undefined && flag != null){
						$("#bookedClassesWrapper").html(bookedClassesThumbsContent(data.subjectCountDatials)+bookedClassesTableContent(data.meetingDetails))
					}else{
						renderBookedClassContent(data, flag, teacherAssignFlag, weekLeftClassCount, assignedTeacherCount);
...
function getRequestForStudentBookClassSlots(formId, moduleId) {
	...
	meetingSlotDTO['meetingDate'] = $("#weeklyClassSlotTable tbody tr td input:checked").attr("data-meeting-start-date-time");
	var cst = $("#weeklyClassSlotTable tbody tr td input:checked").attr("data-class-date")+" "+$("#weeklyClassSlotTable tbody tr td input:checked").attr("data-class-start-time");
	var duration = $("#weeklyClassSlotTable tbody tr td input:checked").attr("data-duration");
	...
	meetingSlotDTO['subjectId'] = $("#weeklyClassSlotTable tbody tr td input:checked").attr("data-suject-id");
	meetingSlotDTO['studentStandardId'] = $("#weeklyClassSlotTable tbody tr td input:checked").attr("data-studentstandard-id");
	meetingSlotDTO['teachUserId'] = $("#weeklyClassSlotTable tbody tr td input:checked").attr("data-teacher-id");
	meetingSlotDTO['schoolPersonId'] = $("#weeklyClassSlotTable tbody tr td input:checked").attr("data-teacher-id");
	...
	$.ajax({
		...
		success: function (data) {
			if (data['status'] == '0' || data['status'] == '2') {
				...
				renderBookClassContent(`${data.studentStandardId}`, `${roleModuleId}`, $("#classPlanCount").attr("data-classPlanCount"),false,`${roleModuleId}`);
...
function showBookClassConfirmationModal(roleModuleId){
	...
	var cn = $("#weeklyClassSlotTable tbody tr td input:checked").attr("data-subject-name");
	var tn = $("#weeklyClassSlotTable tbody tr td input:checked").attr("data-teacher-name");
	var cDate = $("#weeklyClassSlotTable tbody tr td input:checked").attr("data-class-date");
	if($('#weeklyBookClassConfirmationModal').length>0){
		$('#weeklyBookClassConfirmationModal').remove();
	}
	$("body").append(weeklyBookClassConfirmationModal(cst, cet, cDate, cn, tn, roleModuleId));
...
function checkDateAndTimeIsPastOrNot(dateString, timezone, hours) {
    ...
    var providedDate = new Date(dateString);
	var currentDate = new Date(convertUTCToTimezoneAs(getUTCTime(), DATETIME_FORMATTER, timezone).format('MMM DD, YYYY hh:mm:ss a'));
...
function toggleLinkTab(type){
	...
	if(type === 'totalClass'){
		$('#totalClassBtn').addClass('text-white bg-primary').removeClass('text-dark bg-transparent');

		$('#totalClassForWeekBtn').addClass('text-dark bg-transparent').removeClass('text-white bg-primary');
		$('#totalClassSection').removeClass('d-none').addClass('d-flex');
		$('#totalClassForWeekBtnSection').addClass('d-none').removeClass('d-flex');
	} else {
		
		$('#totalClassForWeekBtn').addClass('text-white bg-primary').removeClass('text-dark bg-transparent');
		$('#totalClassBtn').addClass('text-dark bg-transparent').removeClass('text-white bg-primary');
