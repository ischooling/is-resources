async function renderBookClassContent(studentStandardId, showAcademicYearValidationmoduleId, classPlanCount, renderingflag, moduleId){
    
    var payload = {};
    payload['schoolId'] = SCHOOL_ID;
    payload['userTimezone'] = USER_TIMEZONE;
    payload['userId'] = USER_ID;
    var responseData = await getDashboardDataBasedUrlAndPayload(false, true,'book-a-class', payload);
    if(responseData.statusCode == "BUY_A_CLASS"){
        $("#dashboardContentInHTML").html(pageTitleContentForClassError()+renderBuyAClassErrorContent(responseData));
    }else if(responseData.statusCode == "AY_NOT_STARTED"){
        $("#dashboardContentInHTML").html(pageTitleContentForClassError()+renderBuyAClassErrorContent(responseData));
    }else{
        data = await getDetailsForStudentBookaClass(responseData.details.studentStandardId);
        if(!renderingflag){
           $("#classesThumbCotentListWrapper").html(classesThumbCotentList(data)); 
           $("#bookClassContentThumbList").html(classThumbItemListContent(data.subjectList, moduleId));
        }else{
            $("#dashboardContentInHTML").html(pageTitleContent(data, moduleId, false)+getBookClassContent(data, moduleId, responseData.details.showAcademicYearValidation, responseData.details.classPlanCount));
        }
        ...
        `<div class="app-page-title">
            <div class="page-title-wrapper">
                <div class="page-title-heading">
                    ...
                    <div>${data.registerType != 'BATCH'? 'Book a Class'+(data.registerType != "SSP" && (backBtnFlag) && (data.standardId<11 || data.standardId>19 || data.standardId == 18)?"- "+data.subjectName:''):'Book an Extra Class'}</div>
                </div>`;
                if(backBtnFlag){
                    html+=
            ...
            <div class="main-card mb-3 card">
                <div class="card-body">`
                    if(showAcademicYearValidation == "Y"){
                        ...
                        html+=
                        `<div id="totalClassSectionWrapper">`
                            +classesThumbsContent(data)
                        html+=`</div>`
...
function classesThumbCotentList(data){
    ...
    var html=
    `<div class="form-row" id="totalClassSection">
...
function classesThumbsButtletPointContent(classPlanCount, registerType){
    var html=
        `<div class="d-flex flex-wrap bg-light rounded-20 px-2 pt-2 pb-1 mt-3 mb-4">
            ...
            if(registerType=='ONE_TO_ONE' || registerType=='SSP'){
               html+= `<span class="p-1 font-12 px-2 d-inline-flex align-items-center font-weight-semi-bold bg-white rounded-15 mr-1 mb-1"><label class="p-1 d-inline-block bg-primary rounded-circle m-0"></label> <p class="m-0 pl-1">You can book ${classPlanCount == "1"? 'one':'three'} complimentary ${classPlanCount == "1"? 'class':'classes'} per week.</p></span>`;
            }
       html+= ` </div>`;
    return html;
}
        ...
        `<div class="col-xl-4 col-lg-4 col-md-6 col-sm-6 col-12 mb-4">
            <div class="d-flex flex-column h-100">
                <div class="full course-img-wrapper">
                    <img src="${v.img}" class="rounded-top-left-10 rounded-top-right-10"/>    
                </div>
...
function weeklyBookClassModal(data, subjectId, studentStandardId, roleModuleId, modalSize, weekLeftClassCount, assignedTeacherCount){
    
    var html = 
        `<div class="modal fade" id="weeklyBookClassModal" tabindex="-1" data-backdrop="static" role="dialog" aria-modal="true" >
            <div class="modal-dialog h-auto modal-dialog-centered `+modalSize+`" role="document" style="max-width:500px;width:100%; box-shadow:none !important">
                <div class="modal-content h-auto">
                    <div class="modal-header bg-primary py-2 align-items-center">
                        <h5 class="modal-title text-white">${subjectId == 0?(assignedTeacherCount==1?'Book a Class with '+data.studentSubjectList[0].teacherName+'':'Book a Class'):'Book a Class for '+data.subjectName+ ' with '+ data.teacherName}</h5>
                    ...
                    <div class="modal-body">`;
                        if(subjectId != "0"){
                            html+=
                                `<div class="font-weight-bold py-2 d-flex align-items-center justify-content-between mb-2">
                        ...
                        <form class="full" id="bookClassForm">`;
                            if(subjectId == 0){
                                ...
                                `<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 mx-auto">
                                    <select class="form-control" id="elementrySubjectID" name="elementrySubjectID" onchange="changeElementrySubject(this, \'${roleModuleId}\', \'${studentStandardId}\',${data.studentSubjectList[0].teacherName != ''?true:false},${weekLeftClassCount})">
                    ...
                    html+=
                    `</div>
                    <div class="modal-footer ${subjectId != 0? 'justify-content-between':'justify-content-end'}">`
                        if(subjectId != 0){
                            html+=
                            `<p class="font-dark font-weight-bold m-0 invisible">&nbsp;</p>
...
function weeklyBookClassTable(data, subjectId, studentStandardId, roleModuleId){
    var html=
                ...
                while(i<=listSize-1){
                    $.each(data.teacherWeeklyTimeCurrLiveClass.teacherAssignTimeWeekList[i].teacherAllDataInOneList, function(index, value){
                        ...
                        html+='<td class="px-1 py-2 border-0 position-sticky" style="top:0;left:0;z-index:1; background:#f3f3f3">'
                            ...
                            +'<div class="text-center font-12 text-uppercase mt-1" style="line-height:1">'+dayName+'</div>'
                        +'</td>'
                    });
                    i++;
                }
...
function weeklyBookClassTableBody(data, subjectId, studentStandardId, roleModuleId){
    var k=0;
	var listSize=data.teacherWeeklyTimeCurrLiveClass.teacherAssignTimeWeekList.length;
    var html=
        `<tbody>
                    ...
                    $.each(data.teacherWeeklyTimeCurrLiveClass.teacherAssignTimeWeekList[k].teacherAllDataInOneList, function(index, value){
                        html+='<td class="border-0 vertical-align-top">';
                            ...
                            if(value.dayWiseAllData.length>0){
                                var skippedDateExists = data.skipDateList.includes(value.dayWiseAllData[0].bookedDate);
                                if(!skippedDateExists){
                                    $.each(value.dayWiseAllData, function(i, v){
                                        if(v.status=="Booked"){
                                                var isPastDateAndTime = checkDateAndTimeIsPastOrNot(v.bookedStartDate, data.timeZoneName, data.showSlotAfter);
                                                if(isPastDateAndTime){
                                                    ...
                                                    html+=
                                                        `<div class="d-flex align-items-center justify-content-center ">
                                                            <div class="border rounded-5 px-2 py-1 mb-1 text-center" style="width:116px;`+(isPastDate? "background:#f5f5f5;color:#c0c0c0;border-color:#c0c0c0 !important":"background:#ffd6d6;color:#ff0000;border-color:#ff0000 !important")+`">${v.displayStartTime}</div>
                                                        </div>`;
                                                }
...
function weeklyBookClassConfirmationModal(classStartTime, classEndTime, classDate, courseName, teacherName, roleModuleId){
    var html = 
                ...
                <div class="modal-content h-auto">
                    ...
                    <div class="modal-body mx-auto">
                        <div class="d-flex flex-wrap my-1">
                            ...
                            <span class="font-12">${classStartTime}`+' - '+`${classEndTime}</span>
                        </div>
                        <div class="d-flex flex-wrap my-1">
...
function renderBookedClassContent(data, flag, teacherAssignFlag, weekLeftClassCount,assignedTeacherCount){
    var moduleId = $("#bookClassContent").attr("data-moduleId");
    $("#bookClassContent").html(pageTitleContent(data, moduleId, true)+getBookedClassCardContent(data, moduleId, teacherAssignFlag, weekLeftClassCount, assignedTeacherCount));
    $("#bookClassTable").dataTable({
        scrollX:true,
    });
    const monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    let today = new Date();
    let formattedDate = `${monthNames[today.getMonth()]} ${today.getDate()}, ${today.getFullYear()}`;
    ...
    $("#selectedType").on('change',function(){
        if($("#bookClassOneToOne #selectedType").val()=='today'){
            ...
            let startDate = `${monthNames[firstDay.getMonth()]} ${firstDay.getDate()}, ${firstDay.getFullYear()}`;
            let endDate = `${monthNames[lastDay.getMonth()]} ${lastDay.getDate()}, ${lastDay.getFullYear()}`;
            $("#bookClassOneToOne #startDate").val(startDate);
            $("#bookClassOneToOne #endDate").val(endDate);
...
function getBookedClassCardContent(data, moduleId, teacherAssignFlag, weekLeftClassCount,assignedTeacherCount){
    html=
        `<div class="main-card mb-3">`
            +bookClassFilterFormContent(data, moduleId, teacherAssignFlag, weekLeftClassCount, assignedTeacherCount)
            html+=`<div class="full" id="bookedClassesWrapper">`
...
function bookClassFilterFormContent(data, moduleId, teacherAssignFlag, weekLeftClassCount, assignedTeacherCount){
                ...
                <form class="full" id="bookClassOneToOne">
                    <div class="row">
                        ...
                        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12 text-lg-left text-right">
                            ...
                            if(subjectID != 0){
                                html+=`<a href="javascript:void(0)" class="btn btn-lg float-none float-lg-right mb-1 filterBookAClassBtn ${weekLeftClassCount>0?'btn-primary':'btn-dark'}" onclick="bookingSlotModalNew('${subjectID}','${moduleId}','${studentStandardId}',${teacherAssignFlag},${weekLeftClassCount},${assignedTeacherCount});">Book a Class (${weekLeftClassCount})</a>`;
                            }else{
                                html+=`<a href="javascript:void(0)" class="btn btn-primary btn-lg float-none float-lg-right mb-1 filterBookAClassBtn ${weekLeftClassCount>0?'btn-primary':'btn-dark'}" onclick="bookingSlotModalForElementry('${subjectID}','${moduleId}','${studentStandardId}',${teacherAssignFlag},${weekLeftClassCount},${assignedTeacherCount});">Book a Class (${weekLeftClassCount})</a>`;
                            } 
                        html+=`</div>   
...
function bookedClassesTableHeadContent(){
    var tableTitle = ['S. No.', 'Class Timing | Time zone', 'Course Name','Teacher Name', 'Booked Date', 'Class Type', 'Class Status'];
    var html=
        `<thead>
            <tr>`;
                $.each(tableTitle, function(i,v){
                    html+=`<th class="bg-primary text-white font-weight-normal border-bottom-0 vertical-align-middle ${(i==0?'rounded-top-left-10':'')}  ${((i+1)==tableTitle.length?'rounded-top-right-10':'')}">${v}</th>`
                });
            html+=`</tr>
        </thead>`;
    return html;
        ...
        `<tbody>`;
            if(data.length>0){
                $.each(data, function(i,v){
                    html+=
                    `<tr>
                        <td ${((i+1)== data.length? 'class="rounded-bottom-left-10"':'')}>${i+1}</td>    
                        <td>${v.meetingJoinDate} | ${v.startTime} - ${v.endTime} | ${v.timezone}</td>    
                        <td>${v.subjectName}</td>   
                        <td>${v.name}</td>    
                        <td>${v.bookedDate}</td>
                        <td>${v.classTypeExtraOrComp}</td>    
                        <td ${((i+1)== data.length? 'class="rounded-bottom-right-10"':'')}>`;
                            ...
                            }else if(v.meetingResult == 'Missed by Student' || v.meetingResult == 'Missed by Teacher'){
                                html+=`<span class="d-inline-block font-weight-bold ${v.meetingResult == 'Missed by Teacher' ? 'bg-pink':'bg-dark'} text-white px-2 rounded">${v.meetingResult}</span>`;
