function getB2CListHeaderContent(roleAndModule, objRights) {
  ...
  if (
    objRights.discardPermission == true ||
    objRights.defaultUserId == USER_ID
  ) {
    html +=
...
function getB2cLeadList(leaddata, objRights, roleModule) {
  ...
  var html =
    '<input type="checkbox" id="selectLeadAll" class="ml-2" />&nbsp;All' +
    '<select name="leadsPagging" id="leadsPagging" class="ml-1">' +
    '<option value="10" ' +
    (leaddata.recordsPerPage == "10" ? "selected" : "") +
    ">10</option>" +
    '<option value="25" ' +
    (leaddata.recordsPerPage == "25" ? "selected" : "") +
    ">25</option>" +
    '<option value="50" ' +
    (leaddata.recordsPerPage == "50" ? "selected" : "") +
    ">50</option>" +
    '<option value="100" ' +
    (leaddata.recordsPerPage == "100" ? "selected" : "") +
    ">100</option>" +
    "</select>";
  +"<div>" + "</div>";
  for (var i = 0; i < data.length; i++) {
    ...
    if (leads.leadTimeZone != "") {
      ...
      var curentTime = convertTime(
        changeDateFormat(new Date(), "yyyy-mm-dd hh:mm:ss"),
    ...
    if (
      objectRights.discardPermission ||
      USER_ID == leads.assignTo ||
      USER_ID == leads.demoAssignTo
    ) {
      html +=
        ...
        '"/>';
      html +=
        '<input type="checkbox" class="checkLead" id="lead-' +
        leads.leadId +
        '" name="lead-move-another" value="' +
        leads.leadId +
        '" /> ';
    } else {
      html +=
        '<input type="checkbox" disabled="disabled" class="checkLead" id="lead-' +
        leads.leadId +
        '" name="lead-move-another" value="' +
        leads.leadId +
        '" />';
    ...
    html +=
      ...
      '<th class="text-white bold border-bottom-0">School Demo Details <span class="leadDemoTime"></span>&nbsp;&nbsp;' +
      objRights.countryOffsetTimezone +
      " | Status Details</th>";
    if (leads.leadLastCallList != null && leads.leadLastCallList.length > 0) {
      html +=
        '<th class="text-white bold border-bottom-0 text-center" style="width:250px;">Follow Ups</th>';
    } else {
      html +=
        '<th class="text-white bold border-bottom-0 text-center" style="width:250px;">Follow Ups</th>';
    }
    html +=
      '<th class="text-white bold border-bottom-0 rounded-top-right-10 text-center" style="border-top-color:transparent;border-right-color:transparent;width:90px;">Action</th>' +
      "</tr>" +
      "</thead>" +
      ...
      '<tr class="td-border-design ">' +
      '<td style="max-width:320px;min-width: 320px;vertical-align:top;" class="rounded-bottom-left-10 lead-row-' +
      leads.leadId +
      " " +
      ltype +
      "-" +
      (leads.callBadge != "" ? leads.callBadge + "-bg" : "") +
      '">' +
      '<table class="w-100">' +
      "<tbody>" +
      "<tr>" +
      '<th class="border-0 p-1">No.:</th>' +
      '<td class="border-0 p-1 lead-row-td-' +
      leads.leadId +
      " " +
      ltype +
      "-" +
      (leads.callBadge != "" ? leads.callBadge + "-leadno-bg" : "") +
      '" >' +
      leads.leadNo +
      "</td>" +
      "</tr>" +
    ...
    if (leads.userConfirmation === "Y") {
      html +=
        '<tr><th class="border-0 p-1">User Confirmation:</th><td class="border-0 p-1">Yes</td></tr>';
    }

    html +=
      "<tr>" +
      '<th class="border-0 p-1">Lead Status:</th>' +
      '<td class="border-0 p-1 leadlist-status-' +
      leads.leadId +
      '">' +
      (leads.leadStatus != "" ? leads.leadStatus : "N/A") +
      "</td>" +
      "</tr>" +
      "<tr>" +
      '<th class="border-0 p-1">Assigned To:</th>' +
      '<td class="border-0 p-1">' +
      (leads.assignName != "" ? leads.assignName : "N/A") +
      "</td>" +
      "</tr>" +
      "<tr>" +
      '<th class="border-0 p-1">Assigned Date:</th>' +
      '<td class="border-0 p-1">' +
      (leads.createdDateStr != "" ? leads.createdDateStr : "N/A") +
      "</td>" +
      "</tr>" +
      "<tr>" +
      '<th class="border-0 p-1">Added By:</th>' +
      '<td class="border-0 p-1">' +
      (leads.userName != "" ? leads.userName : "N/A") +
      "</td>" +
      "</tr>";
    if (leads.leadSupportToName != "") {
      html +=
        ...
        '<td class="border-0 p-1">' +
        (leads.leadSupportToName != "" ? leads.leadSupportToName : "N/A") +
        "</td>" +
        "</tr>";
    }
    ...
    if (leads.leadSourceId == 4) {
      html +=
        ...
        '<td class="border-0 p-0 font-12 vertical-align-top">' +
        (leads.utmSource != "" ? leads.utmSource : "N/A") +
        "</td>" +
        "</tr>" +
        '<tr class="border-bottom">' +
        '<th class="border-0 p-0 font-12 vertical-align-top pr-1">Ad:</th>' +
        '<td class="border-0 p-0 font-12 vertical-align-top">' +
        (leads.utmMedium != "" ? leads.utmMedium : "N/A") +
        "</td>" +
        "</tr>" +
        '<tr class="border-bottom">' +
        '<th class="border-0 p-0 font-12 vertical-align-top pr-1">Ad Set:</th>' +
        '<td class="border-0 p-0 font-12 vertical-align-top">' +
        (leads.utmDescription != "" ? leads.utmDescription : "N/A") +
        "</td>" +
        "</tr>" +
        '<tr class="border-bottom">' +
        '<th class="border-0 p-0 font-12 vertical-align-top pr-1">Campaign:</th>' +
        '<td class="border-0 p-0 font-12 vertical-align-top utmCampaign-' +
        leads.leadId +
        '">' +
        (leads.utmCampaign != "" ? leads.utmCampaign : "N/A") +
        "</td>" +
        "</tr>" +
        '<tr class="border-bottom">' +
        '<th class="border-0 p-0 font-12 vertical-align-top pr-1">Is Organic:</th>' +
        '<td class="border-0 p-0 font-12 vertical-align-top">' +
        (leads.utmTerm != "" ? leads.utmTerm : "N/A") +
        "</td>" +
        "</tr>";
      if (leads.leadPlatform != "") {
        html +=
          ...
          '<td class="border-0 p-0 font-12 vertical-align-top"><img class="report-icon" src="' +
          PATH_FOLDER_IMAGE2 +
          leads.leadPlatform +
          ".png" +
          SCRIPT_VERSION +
      ...
      if (leads.fbImageUrl != "") {
        html +=
          ...
          '<a href="' +
          leads.fbImageUrl +
          '" class="btn btn-primary btn-sm" target="_blank">View Form Image</a>' +
          "</td>";
        +"</tr>";
      }
    } else {
      html +=
        ...
        '<td class="border-0 p-0 font-12 utmCampaign-' +
        leads.leadId +
        '">' +
        (leads.utmCampaign != "" ? leads.utmCampaign : "N/A") +
        "</td>" +
    ...
    if (objRights.discardPermission || USER_ID == leads.assignTo) {
      html +=
        ...
        '<select class="selectcampain" name="campainName" id="campainName_' +
        leads.leadId +
        '">' +
      ...
      for (let c = 0; c < campaignList.length; c++) {
        ...
        html +=
          ...
          campaign.value +
          '" ' +
          (leads.utmCampaign == campaign.value ? "selected" : "") +
          ">" +
          campaign.value +
          " (" +
          campaign.extra +
      ...
      html +=
        "</select>" +
        '<button class="ml-2 mr-1 btn btn-sm btn-info" id="saveCampaign" onclick="saveCampaignLead(\'' +
        leads.leadId +
        "','campainName_" +
        leads.leadId +
        "', 'new-leadcampaign');\">Save</button>" +
        "</div>" +
        "</td>" +
        "</tr>";
    }
    ...
    if (objRights.discardPermission || USER_ID == leads.assignTo) {
      html +=
        '<div role="" class="mb-2 btn-group-sm btn-group-toggle text-left" data-toggle="buttons">' +
        '<label class="btn btn-outline-success mr-1 ' +
        (leads.leadCategory == "Hot" ? "active" : "") +
        '">' +
        '<input type="radio"  name="leadCategory' +
        leads.leadId +
        '" id="hot' +
        leads.leadId +
        '" value="Hot" autocomplete="off" ' +
        (leads.leadCategory == "Hot" ? "checked" : "") +
        " onchange=\"saveCategoryLead('" +
        leads.leadId +
        "','hot" +
        leads.leadId +
        "');\"> Hot" +
        ...
        (leads.leadCategory == "Cold" ? "active" : "") +
        '">' +
        '<input type="radio"  name="leadCategory' +
        leads.leadId +
        '" id="cold' +
        leads.leadId +
        '" value="Cold" autocomplete="off" ' +
        (leads.leadCategory == "Cold" ? "checked" : "") +
        " onchange=\"saveCategoryLead('" +
        leads.leadId +
        "','cold" +
        leads.leadId +
        ...
        (leads.leadCategory == "Warm" ? "active" : "") +
        '">' +
        '<input type="radio" name="leadCategory' +
        leads.leadId +
        '" id="warm' +
        leads.leadId +
        '" value="Warm" autocomplete="off" ' +
        (leads.leadCategory == "Warm" ? "checked" : "") +
        " onchange=\"saveCategoryLead('" +
        leads.leadId +
        "','warm" +
        leads.leadId +
    ...
    html +=
      ...
      '<td class="border-0 p-1">' +
      (leads.standardName != ""
        ? leads.standardName.replace("Grade", "")
        : "N/A");
    if (
      objRights.discardPermission ||
      USER_ID == leads.assignTo ||
      ...
      html +=
        ...
        "<a href=\"javascript:void(0);\" onclick=\"callLeadsByLeadId('leadDataPopupForm','" +
        leads.leadId +
        "','" +
        USER_ID +
        "','edit','leadPopupForm','B2C'," +
        objRights.discardPermission +
        ');" >' +
    ...
    html +=
      ...
      '<td class="border-0 p-1">' +
      (leads.cityName != "" ? leads.cityName : "N/A") +
      " | " +
      (leads.countryName != "" ? leads.countryName : "N/A") +
      "</td>" +
      "</tr>" +
      "<tr>" +
      '<th class="border-0 p-1" style="width:165px">Phone:</th>' +
      '<td class="border-0 p-1">' +
      (leads.isdCode != "" ? leads.isdCode : "") +
      " " +
      (leads.phone != "" ? leads.phone : "N/A");
    if (leads.isdCode != "") {
      html += "<span>";
      html +=
        '<a href="https://api.whatsapp.com/send?phone=' +
        (leads.phoneIsd != "" ? leads.phoneIsd : "") +
        '" target="_target" class="position-relative">';
      html +=
      ...
      if (leads.whatsAppVerifiedStatus == "N") {
        ...
        if (leads.whatsAppScbStatus == "N") {
          html +=
            '<span style="left: 9px;color: black;font-size: 12x;position: absolute;top: -7px;" data-toggle="tooltip" data-placement="top" data-original-title="Wati Message Sent"><i class="fa fa-minus-circle"></i></span>';
        } else {
          if (leads.whatsAppVerifiedStatus == "Y") {
            html +=
              '<span style="left: 9px;color: green;font-size: 12x;position: absolute;top: -7px;" data-toggle="tooltip" data-placement="top" data-original-title="Wati Message Sent"><i class="fa fa-check-circle"></i></span>';
          } else if (leads.whatsAppVerifiedStatus == "N") {
            html +=
              '<span style="left: 9px;color: red;font-size: 12x;position: absolute;top: -7px;" data-toggle="tooltip" data-placement="top" data-original-title="Wati Message Not Sent"><i class="fa fa-times-circle"></i></span>';
          }
        }
      }
      html += "</a>" + "</span>";
    ...
    if (leads.phoneNoAlter != "") {
      html +=
        (leads.phoneNoAlter != "" ? leads.isdCodeAlter : "") +
        " " +
        (leads.phoneNoAlter != "" ? leads.phoneNoAlter : "");
      html +=
        '<a href="https://api.whatsapp.com/send?phone=' +
        (leads.altrphoneIsd != "" ? leads.altrphoneIsd : "") +
        '" target="_target"> <img src="' +
        PATH_FOLDER_IMAGE +
        'watsapp-icon.png" width="16px" /></a>';
    }
    ...
    html +=
      ...
      '<td class="border-0 p-1">' +
      (leads.email != "" ? leads.email : "N/A");
    if (leads.email != "") {
      if (leads.verifiedEmail > 0) {
        html +=
          '<span style="color:green;font-size:15px;" data-toggle="tooltip" data-placement="top" data-original-title="Email Verified"><i class="fa fa-check-circle"></i></span>';
      } else if (leads.verifiedEmail < 1) {
        html +=
          '<span style="color:green;font-size:15px;"><i class="fa fa-remove"></i></span>';
      }
    }
    html += "<br/>";
    ...
    html +=
      ...
      "</tr>" +
      "<tr>" +
      '<th class="border-0 p-1" style="width:165px;background-color:' +
      (leads.leadStatus == "Converted" ? "#baf3cd" : "#f0ddc1") +
      ';">Lead To Enrolled:</th>' +
      '<td class="border-0 p-1">' +
      (leads.leadTotalDay != "" ? leads.leadTotalDay : "0") +
      "</td>" +
      "</tr>" +
      "<tr>" +
      '<th class="border-0 p-1">Lead’s Current Time:</th>' +
      '<td class="border-0 p-1 bold">' +
      currentTimeStr +
      "</td>" +
      "</tr>";
    if (objRights.discardPermission) {
      ...
      html +=
        ...
        '<td class="border-0 p-1">' +
        (leads.curentStage != "" ? leads.curentStage : "N/A") +
        "</td>" +
        "</tr>";
    }
    html +=
      "<tr>" +
      "<tr>" +
      '<td colspan="2"  class="border-0 p-1 leadtagstatus_' +
      leads.leadNo +
      '"></td>' +
      "</tr>" +
      ...
      '<p class="m-0">' +
      (leads.remarks != "" ? leads.remarks : "N/A") +
      "</p>" +
      "</div>" +
      "</div>" +
      "</td>" +
    ...
    html +=
      ...
      '<td class="border-0 p-1">' +
      (leads.leadDemoIstDate != "" ? leads.leadDemoIstDate : "N/A") +
      "</td>" +
      "</tr>" +
      '<tr  class="" >' +
      '<th class="border-0 p-1">Status:</th>' +
      '<td class="border-0 p-1">' +
      (leads.demoStatus != "" ? leads.demoStatus : "N/A") +
      "</td>" +
      "</tr>" +
      '<tr  class="" >' +
      '<th class="border-0 p-1">Assigned To:</th>' +
      '<td class="border-0 p-1">' +
      (leads.demoAssignName != "" ? leads.demoAssignName : "N/A") +
      "</td>" +
      "</tr>" +
      "</tbody>" +
      "</table>" +
    ...
    if (objRights.discardPermission) {
      html +=
        ...
        '<td class="border-0 p-1">' +
        (leads.followup != "" ? leads.followup : "N/A") +
        "</td>" +
        "</tr>";
    }
    html +=
      ...
      '<td class="border-0 p-1 nextSchedule-' +
      leads.leadId +
      " " +
      ltype +
      "-" +
      (leads.callBadge != "" ? leads.callBadge + "-leadno-bg" : "") +
      '">' +
      (leads.leadFollowDate != "" ? leads.leadFollowDate : "N/A") +
      "</td>" +
      "</tr>";
    if (leads.tantativeDate) {
      html +=
        ...
        '<td class="border-0 p-1">' +
        (leads.tantativeDate != "" ? leads.tantativeDate : "N/A") +
        "</td>" +
        "</tr>";
    }
    ...
    html +=
      ...
      '<td class="border-0 p-1  nextFollow-' +
      leads.leadId +
      " " +
      ltype +
      "-" +
    ...
    if (leads.leadFollowUser) {
      html +=
        ...
        '<td class="border-0 p-1 ">' +
        (leads.leadFollowUser != "" ? leads.leadFollowUser : "N/A") +
        "</td>" +
        "</tr>";
    }
    html +=
      ...
      '<td class="border-0 p-1 leadlist-remark-' +
      leads.leadId +
      " " +
      ltype +
      "-" +
      (leads.callBadge != "" ? leads.callBadge + "-leadno-bg" : "") +
      '">' +
      (leads.followupRemark != "" ? leads.followupRemark : "N/A") +
      // +'<div class="dropdown d-inline-block" style="position: inherit;">'
    ...
    if (
      ...
      USER_ID == leads.assignTo ||
      USER_ID == leads.demoAssignTo ||
      USER_ID == leads.leadSupportTo
    ) {
      let isRemarkMandatory =
        leaddata.remarkMendatory && leaddata.minRemarkCount > 0;
      html +=
        "<tr>" +
        ...
        '" id="leadStatus-' +
        leads.leadId +
        '" class="leadStatus-followup" style="width:200px !important; height:30px;">' +
        '<option value="">Select Status</option>';
      ...
      html +=
        ...
        (isRemarkMandatory ? "lead_list_remarks " : "") +
        'remarks" ' +
        'data-leadid="' +
        leads.leadId +
        '" ' +
        ...
        (isRemarkMandatory
          ? 'minlength="' + leaddata.minRemarkCount + '" required'
          : "") +
        "></textarea>" +
        (isRemarkMandatory
          ? '<small id="leadListRemarksCounter_' +
            leads.leadId +
            '" class="text-muted">0 / ' +
            leaddata.minRemarkCount +
            "</small>"
          : "") +
        '<div class="suggestionslead" id="suggestions-' +
        leads.leadId +
        ...
        "<tr>" +
        "<td>" +
        '<button class="ml-2 mr-1 btn btn-sm btn-info float-right" id="updateFollowup" onclick="submitFollowupSaveFromLeadList(\'followupSaveForm\', \'' +
        leads.leadId +
        "', '" +
        objRights.leadType +
        "', '" +
    ...
    if (leads.leadLastCallList != "" && leads.leadLastCallList.length > 0) {
      html +=
        '<ul class="follow-up-accordian m-0 p-0 overflow-auto followup-remark-' +
        leads.leadNo +
    ...
    for (var l = 0; l < leads.leadLastCallList.length > 0; l++) {
      ...
      html +=
        '<li class=" ' +
        (l == 0 ? "follow-up-accordian-active" : "") +
        '">' +
        '<span class="cursor follow-up-no text-primary p-2 text-center border-primary full bold">' +
        leadCall.leadFollowStatus +
        ' <i class="fa ' +
        (l == 0 ? "fa-angle-up" : "fa-angle-down") +
        ' float-right" style="line-height: 20px;"></i></span>' +
        '<div class="follow-up-content text-center" style="' +
        (l == 0 ? "display: block" : "") +
        '">' +
        ...
        '<td class="p-1 border-0" id="connectedTh">' +
        leadCall.callscheduleDateString +
        "</td>" +
        "</tr>" +
        "<tr>" +
        ...
        '<td class="p-1 border-0" id="connectWith">' +
        leadCall.toCall +
        "</td>" +
        "</tr>" +
        "<tr>" +
        '<th class="p-1 border-0">Lead Status & Category:</th>' +
        '<td class="p-1 border-0" id="leadFollowStatusCategory">' +
        leadCall.leadFollowStatus +
        "</td>" +
        "</tr>" +
        "<tr>" +
        ...
        (leadCall.nextCallscheduleDateString != ""
          ? leadCall.nextCallscheduleDateString
          : "") +
        "</td>" +
        "</tr>" +
        ...
        '<td class="p-1 border-0 text-justify" id="callRemark" style="max-width: 250px;">' +
        (leadCall.followRemarks != "" ? leadCall.followRemarks : "N/A") +
        "</td>" +
        "</tr>" +
        "</table>" +
        "</div>" +
    ...
    html +=
      "</ul>" +
      "</td>" +
      '<td class="rounded-bottom-right-10 text-center pt-3 lead-row-' +
      leads.leadId +
      " " +
      ltype +
      "-" +
    ...
    if (
      ...
      if (leads.leadStatus == "Unassigned") {
        html +=
          '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" data-original-title="Update" onclick="callGetOpenFollowup(\'followupSaveForm\',\'' +
          leads.leadId +
          "','" +
          USER_ID +
          "','edit','" +
          objRights.currentPage +
          "','leadFollowupForm','B2C','Y'," +
          leaddata.remarkMendatory +
          "," +
          leaddata.minRemarkCount +
        ...
        if (
          objRights.discardPermission &&
          objRights.leadFrom == "ARCHIVEDLEAD" &&
          roleModule.updated == "Y"
        ) {
          var disFun =
            "discardLeadsData('" +
            leads.leadId +
            ...
            "','B2C','new-leads')";
          var discardFun =
            "return showNewDiscardLeadModelFunction('" +
            disFun +
            "','" +
            leads.LeadSourceName +
            "','" +
            ...
            "')";
          html +=
            '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" data-original-title="Discard" onclick="' +
            discardFun +
            '"><i class="fa fa-trash" aria-hidden="true" style="font-size:16px;margin-bottom:4px;padding:4px;"></i></a><br/>';
        }
      } else {
        ...
        if (roleModule.updated == "Y") {
          html +=
            '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" data-original-title="Clone" onclick="callLeadsByLeadId(\'leadDataPopupForm\',\'' +
            leads.leadId +
            "','" +
            USER_ID +
            "','addLeadClone','leadPopupForm','B2C'," +
            objRights.discardPermission +
            ');" >' +
        ...
        if (roleModule.updated == "Y") {
          html +=
            '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" data-original-title="Partner with B2B" onclick="callLeadsByLeadId(\'leadDataPopupB2BForm\',\'' +
            leads.leadId +
            "','" +
            USER_ID +
            "','addLeadClone','leadPopupB2BForm','B2C-B2B'," +
            objRights.discardPermission +
            ');" >' +
            '<i class="fa fa-handshake" aria-hidden="true" style="font-size:16px;margin-bottom:4px;padding:4px;"></i></a><br/>';
        }
        html +=
          '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" data-original-title="Activity logs" onclick="getAsPost(\'/dashboard/lead-followup-log-data?moduleId=moduleId=' +
          objRights.moduleId +
          "&leadNo=" +
          leads.leadNo +
          "&leadType=B2C')\">" +
        ...
        html +=
          '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" data-original-title="Update chat support" onclick="renderChatContent(\'' +
          objRights.discardPermission +
          "','" +
          USER_ID +
          "','" +
        ...
        if (
          ...
          html +=
            ...
            discardFun +
            '"><i class="fa fa-trash text-danger"></i>&nbsp;Discard</a><br/>';
        }
        ...
        html +=
          '<a href="' +
          leads.demoSendUrl +
          '" data-toggle="tooltip" data-placement="top" data-original-title="Book School Demo with ' +
          (leads.demoAssignName != ""
            ? leads.demoAssignName
            : leads.assignName) +
          '" target="_blank"><i class="fa fa-bookmark" aria-hidden="true" style="font-size:16px;margin-bottom:4px;padding:4px;"></i></a><br/>';
        html +=
          '<a href="' +
          leads.demoSendUrlForAll +
          '" data-toggle="tooltip" data-placement="top" data-original-title="Book School Demo for other counselor" target="_blank"><i class="fa fa-bookmark" aria-hidden="true" style="font-size:16px;margin-bottom:4px;padding:4px;"></i></a><br/>';
        html +=
          '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" data-original-title="WhatsApp chat log" onclick="callOpenWatsAppMessage(\'watsAppMsgModal\',\'' +
          leads.leadId +
          '\');"><img src="' +
          PATH_FOLDER_IMAGE2 +
          "leadlist_icons/WhatsApp.svg" +
          SCRIPT_VERSION +
        ...
        if (leads.whatsAppVerifiedStatus == "NA") {
          ...
          html +=
            '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" data-original-title="Wati Logs" style="display:' +
            displayCss +
            ';" id="wati_logs_link_' +
            leads.leadId +
            '" onclick="getWatiLogs(\'' +
            ...
            '<img src="' +
            PATH_FOLDER_IMAGE2 +
            "leadlist_icons/Wati.svg" +
            SCRIPT_VERSION +
            '" style="width:26px; margin-bottom: 4px;padding:4px;" /></a>';
        }
        if (leads.zadarmaCount > 0) {
          html +=
            '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" data-original-title="Zadarma Logs" onclick="getZadarmaLogs(\'' +
            leads.phoneIsd +
            '\')"><img src="' +
            PATH_FOLDER_IMAGE2 +
            "leadlist_icons/Zadarma.svg" +
            SCRIPT_VERSION +
            '" style="width:26px; margin-bottom: 4px;padding:4px;" /></a><br/>';
        }
        if (leads.emailBroadcastCount > 0) {
          html +=
            '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" data-original-title="Email Broadcast Logs" onclick="getEmailBroadcastLogs(\'' +
            leads.email +
            "','" +
            (leads.fname != "" ? leads.fname : "N/A") +
            " " +
            ...
            "','" +
            leads.leadId +
            '\')"><i class="fa fa-envelope" aria-hidden="true" style="font-size:16px;margin-bottom:4px;padding:4px;"></i></a><br/>';
        }
      }
    } else {
      html += "N/A";
...
function b2cleadsPagging(leaddata, objRights) {
  ...
  if (noOfPages > 1) {
    ...
    if (currentPage != 1) {
      html +=
        '<li class="page-item">' +
        '<a class="page-link" href="javascript:void(0);" onclick="clickTotalLeads(\'' +
        objRights.clickFrom +
        "-" +
        objRights.clickUserid +
        "', '" +
        (currentPage - 1) +
        "','" +
        objRights.clickByLead +
        "','" +
        objRights.leadFrom +
        "');\">Previous</a>" +
        "</li>";
    ...
    for (let p = 1; p <= noOfPages; p++) {
      if (
        ...
        html +=
          '<li class="page-item">' +
          '<a href="javascript:void(0);" onclick="clickTotalLeads(\'' +
          objRights.clickFrom +
          "-" +
          objRights.clickUserid +
          "' ,'" +
          ...
          objRights.leadFrom +
          '\');" class="page-link ' +
          (p == currentPage ? "page-link-active" : "") +
          '">' +
          p +
          "</a>" +
          "</li>";
