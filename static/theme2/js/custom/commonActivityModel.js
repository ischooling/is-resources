						...
						if(tt!="theme1"){
							html+=
								`<div class="d-flex justify-content-between align-items-center w-100">
									...
									<button type="button" class="btn btn-sm btn-pill bg-transparent" data-dismiss="modal" style="box-shadow: 0px 0px; padding: 8px; font-weight: bold;"><i class="fa fa-times" style="font-size: 18px; color: #FFF;" aria-hidden="true"></i></button>
								</div>`;
						}else{
							html+=
								`<div class="d-flex justify-content-between align-items-center w-100">
...
function calendarMeetingLinkValidateActivityStudent(url, response){
	...
	var html = 
		`<div id="classJoinWaringDiv">
			...
			<a target="_blank" id="classJoinWaring" href="${url}" onclick="autodiposeModel('classJoinInSameWindowModal')" class="btn btn-pill mt-2 mx-auto" style="background-color: #027FFF; box-shadow: 0px 0px; padding: 5px 10px; display: flex; width: max-content; color: white;">Join Class Now</a>
			...
			<div class="full text-right mt-3">
				...
				<button value="${url}" class="btn btn-pill text-white mx-auto align-items-center" style="background:${getColorCode}" onclick="copyURL('copyURL0','copy-msg-0')"><i class="fa fa-clone" style="font-size: 14px;"></i>&nbsp;Copy Class Link</button>
			</div>
		</div>`
	return html;
}
