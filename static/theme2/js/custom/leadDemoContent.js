	...
	var html = 
		...
		+'<div class="main-card mb-3 card">'
			...
			+'<div class="w-100" style="gap:0.5rem">'
				+' <form action="javascript:void(0);" class="d-flex align-items-center flex-nowrap flex-wrap justify-content-end custom-field-scope" id="demoSearchForm" name="demoSearchForm" autocomplete="off" style="gap:0.5rem">'
					+'<div class="custom-field mb-0" style="width:250px">'
						+'<select name="assignToSearch" id="assignToSearch" class="form-control"></select>'
						+'<label for="assignToSearch" class="m-0 d-block mb-0">Assign To</label>'
					+'</div>'
					+'<div class="custom-field mb-0" style="width:fit-content">'
						+'<select class="form-control" id="searchDaywiseDemo" name="searchDaywiseDemo" style="width:fit-content">'
							+'<option value="Day">Today</option>'
							+'<option value="Week">Week</option>'
							+'<option value="CUSTOM" >Custom</option>'
						+'</select>'
						+'<label for="searchDaywiseDemo" class="m-0 d-block mb-0">View Type</label>'
					+'</div>'
					+'<div class="hidestudentdate">'
						+'<div class="d-flex align-items-center flex-nowrap" style="gap:0.5rem">'
							+'<div class="custom-field mb-0">'
								+'<input type="text" name="dataDemoStartDate" class="form-control form-control-sm" id="dataDemoStartDate" placeholder="Start Date" style="width:120px" readonly onkeydown="return false" />'
								+'<label for="dataDemoStartDate" class="m-0 d-block mb-0">Start Date</label>'
							+'</div>'
 ...
 function getCalendarDemoAvailabilityTable(formId, userId, data){
	...
	if(data.slotType=='Day'){
	 ...
	 for (let i = 0; i < timeSlotList.length; i++) {
		...
		let time24h = leadConvertTo24Hour(timedates);
