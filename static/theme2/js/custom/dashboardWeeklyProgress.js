...
function uploadSWPRCSV(formId, studentId, standardId) {
	hideMessageTheme2('');
	if($('#semesterId').val()==''){
		showMessageTheme2(0, 'Please select session');
		return false;
	}
	if($('#reportId').val()==''){
		showMessageTheme2(0, 'Please select Report Type');
		return false;
	}
	if($('#frequencyDate').val()==''){
		showMessageTheme2(0, 'Please select weekly report date');
		return false;
	}
	if($('#fileupload1Hash').val()==''){
		showMessageTheme2(0, 'Please upload csv file');
		return false;
	}
...
function showWarningMessageForGenerate(warningMessage, functionName){
	if($('#startReportDate').val()==''){
		showMessageTheme2(0, 'Please select Start report date');
		return false;
	}

	if($('#endReportDate').val()==''){
		showMessageTheme2(0, 'Please select End report date');
		return false;
	}
	...
	if(startTime > endTime){
		showMessageTheme2(0, 'Report start date must be less than Report end date.');
		return false;
	}
	if(functionName==''){
