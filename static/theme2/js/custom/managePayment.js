            ...
            if (data['status'] == '0' || data['status'] == '2' || data['status'] == '3') {
                ...
                if (data.advancePaymentSearchResponseDTO != null && data.advancePaymentSearchResponseDTO != undefined && data.advancePaymentSearchResponseDTO.length > 0) {
                    //if(data.advancePaymentSearchResponseDTO.length > 0){
                        ...
                        $('#' + elementId + ' tbody').html(await getAdvancePaymentSearchResult(formId, data, moduleId));
...
function addExternalPayment(formId, paymentTitle) {
    ...
    // $('#studentId1').prop('onblur','getStudentDetailsForPayment(\'addStudentPaymentForm\',\'true\')');
