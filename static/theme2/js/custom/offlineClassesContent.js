	...
	$.each(result, function(k, v) {
		...
		if(v.status=='Deleted'){
			...
			action='<div class="btn-group">'
						+'<button type="button" class="btn btn-primary dropdown-toggle  btn-sm" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-toggle="tooltip" title="Action">'
...
function getOfflineClassContent(title, roleAndModule, schoolId, userId, role){
	...
	+'<div class="page-title-wrapper">'
		...
		if(roleAndModule.added=='Y'){
			...
			html+='<a href="javascript:void(0);" onClick="showOfflineClassModel(\'offlineClassAdd\', \'offlineClassesModal\')" class="btn btn-primary meetingSlotAdd ml-1">Add Links</a>';
			html+='</div>';
			html+='</div>';
		}
		html+='</div>'
