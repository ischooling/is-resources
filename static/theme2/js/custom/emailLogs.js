        ...
        success: function (data) {
            ...
            if (data['status'] == '0' || data['status'] == '2') {
                ...
                $('#emailLogsTable').dataTable({
                    ...
                    scrollX:true,
                    bFilter:false,
                    autoWidth: false, // Important to disable automatic resizing
                    columnDefs: [
...
function emailLogTable(data) {
    var html = '<table class="table table-bordered table-striped border-radius-table font-12" id="emailLogsTable" style="width:100%;min-width:1100px">'
        ...
        + '<th>CC</th>'
        + '<th>BCC</th>'
        + '<th style="max-width:200px">Mail Subject</th>'
        + '<th>Attachments</th>'
        + '<th>Service Provider</th>'
        + '<th>Date Time (UTC +5:30)</th>'
        + '<th>Mail Status</th>'
        + '<th>Action</th>'
        + '</tr>'
        + '</thead>'
...
function getEmailVerifyByEmail() {
    ...
    if (!validateEmail(email)) {
        showMessageTheme2(0, 'Email is either empty or invalid', 'studEmailIdError');
        return false;
    }
    if (email != '') {
        var data={};
...
function emailVerificationTable(data) {
    var html=`
        <table class="table table-bordered table-striped border-radius-table font-12 responsive nowrap" id="emailVTable">
            <thead class="bg-primary text-white">
                <tr>
                    <th>Email</th>
...
function getVTableData(data) {
    ...
    if (data.emailStatus === 'Verified') {
        button = `<td>
                    <button onClick="getChangeEmailVerification(0)" class="btn btn-primary ">Unverify</button>
                  </td>`;
    } else {
        button = `<td>
                    <button onClick="getChangeEmailVerification(1)" class="btn btn-primary ">Verify</button>
                  </td>`;
    }
    var row = 
        `<tr>
...
function resizeIframe(obj) {
    $("#emailTemplate").css("height","150px")
    // alert($("#emailTemplate").height());
    // alert(obj.contentWindow.document.documentElement.scrollHeight);
    obj.style.height = obj.contentWindow.document.documentElement.scrollHeight + 'px';
  }

function resetFormEmail(formId){
    $('#' + formId)[0].reset();
    $("#"+formId+" #getEmail").val("");
