...
function getUserAvailabilityHtml(data, userId, prestartTime, preendTime,userRoleId, discardPermission){
	...
	if(specific_tr==0){
		...
		htmlCal=htmlCal+'<thead>';
		htmlCal=htmlCal+'<tr>'; 
			htmlCal=htmlCal+'<th class="border-right-0 border-left-0">Specific Date</th>';
			if(userRoleId==3){
				htmlCal=htmlCal+'<th class="border-right-0 border-left-0">Live Classes Slot</th>';
			}else{
				htmlCal=htmlCal+'<th class="border-right-0 border-left-0">&nbsp;</th>';
			}
			htmlCal=htmlCal+'<th width="215px" class="border-right-0 border-left-0">';
	...
	if(timeAvailableList!=null && timeAvailableList.length>0){
		for (let i = 0; i < timeAvailableList.length; i++) {
			...
			htmlCal=htmlCal+'<td class="border-left-0 border-right-0">'+userdates.selectDate+'</td>'; 
			htmlCal=htmlCal+'<td class="border-left-0 border-right-0" id="specific_td_'+rownum+'">';
			...
			htmlCal=htmlCal+'</td>';  
			if(userdates.dayTimeType=="Unavailable"){
				htmlCal=htmlCal+'<td class="border-left-0 border-right-0"></td>';
			}else{
				htmlCal=htmlCal+'<td class="'+(userdates.dataType!='Y'? 'new-add-availibality-time-td':'')+' border-left-0 border-right-0">'+userdates.totalTimeHrsMins+'</td>';
			}

			if(userRoleId!=3){
				htmlCal=htmlCal+'<td class="border-left-0 border-right-0"><a href="javascript:void(0);" class="click-to-catalog btn btn-sm" style="font-size:24px !important" onclick="'+onFuncDate+'"><i class="lnr lnr-cross"></i></a></td>';   
			}else{
				// if(userdates.dataType!='Y' && USER_ROLE_ID==3){
			...
			if(userTimeA.dayid==days.dayId){
				...
				if(userTimeA.dayCheck=='N'){
					...
					if(userRoleId != 3){
						...
						htmlw=htmlw+'<select class="form-control font-12 toTime" disabled data-id="'+days.dayName+'-'+days.dayId+'" data-checkid="'+days.dayId+'" id="'+days.dayName+'-end-time-dropdown-'+ttt+'" onchange="'+toFunc+'" data-status="Y">';
					}
					if(timeDropdown.length>0){
