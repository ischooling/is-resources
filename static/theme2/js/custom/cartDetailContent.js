function renderCardDetailsContent(userId){
    if ($('head script[src*="buyExtraClassContent.js"]').length < 1) {
       $("head").append(`
            <script id="buyExtraClassContentJS" type="text/javascript" src="${PATH_FOLDER_JS2}${RESOURCES_FROM_MIN_LOCATION}custom/buyExtraClassContent.js${SCRIPT_VERSION}"></script>
            <script id="buyExtraClassJS" type="text/javascript" src="${PATH_FOLDER_JS2}${RESOURCES_FROM_MIN_LOCATION}custom/buyExtraClass.js${SCRIPT_VERSION}"></script>
        `)
    }
                            ...
                            <div class="agree">`;
                                ...
                                html+=`<ol style="list-style: auto;" class="pl-4">`;
                                    if(SCHOOL_ID==5 && (data.planStatus === 'b'  || data.planStatus === 'bd')){
                                        ...
                                        <li>In case you want to change the date and/or timings of your Extra Classes, you will have to inform ${schoolSettingsOffice.schoolName} via mail at least 7 days in advance.</li>
                                        ...
                                        <li>The class will be compensated only if it is canceled by ${schoolSettingsOffice.schoolName}. The compensation will occur only through policies of ${schoolSettingsOffice.schoolName}. You cannot claim a refund of fees in such cases.</li>`
                                        ...
                                        <li>In case you want to change the date and/or timings of your Extra Class(s), you will have to inform ${schoolSettingsOffice.schoolName} via mail at least 7 days in advance.</li>
                                        ...
                                        <li>The class will be compensated only if it is canceled by ${schoolSettingsOffice.schoolName}. The compensation will occur only through policies of ${schoolSettingsOffice.schoolName}. You cannot claim a refund of fees in such cases.</li>
                                        <li>Students (or their parents/guardians) are responsible for regularly checking the website for any upcoming notifications. ${schoolSettingsOffice.schoolName} will not send notifications or updates separately to students (or their parents/guardians)</li>
                                        <li>${schoolSettingsOffice.schoolName} reserves the right to amend, limit or revoke any offers or terms at any time prior to purchase and accepts no responsibility for any technical issues resulting in the failure to pay.</li>`
