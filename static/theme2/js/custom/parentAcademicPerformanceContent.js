...
function getParentAcademicPerformanceRowsHtml(rows){
    ...
    $.each(rows || [], function(index, row){
        var courseHtml = row.courseName.split("Grade")[0] || "N/A";
        if(row.lmsEnrollmentId && row.lmsCourseId){
            courseHtml = `<a href="javascript:void(0)" onclick="parentAcademicPerformanceOpenProgressDetail('${row.studentUserId}','${row.lmsEnrollmentId}','${row.lmsCourseId}')">${courseHtml}</a>`;
        }
        rowsHtml += `
            <tr>
